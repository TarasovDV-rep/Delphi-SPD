...
begin
     ...
     try
      ...
      // Проверим наличие каталога JRN и arh
      ...
      // Проверим наличие текущего файла
      ...
      //Если 1-й ЧАС  то заархивируем все, что есть положим в jrn\mes_arh
      if HourOf(Now) = 1 then
      begin
       //Имя архива - год+мес+день
       arhname := IntToStr(YearOf(Now)) + IntToStr(MonthOf(Now))+
                  IntToStr(DayOf(Now))+'.zip';
       zip := TVCLZip.Create(Application);
       //Если такой архив есть то не надо - уже!
      ...
      //Проверим кол-во архивов - лишние удалить
...
//      //Откроем для добавления
//      Append(ff);
//      Writeln(ff,DateTimeToStr(Now)+' -> '+ #13+#10+SysSoob);
//      CloseFile(ff);

     except
     // SaveTekJrn('Ош. при записи:'+SysSoob);
     end;
...
begin
      ...
      SaveTekJrn(DateTimeToStr(now)+' : Старт программы.');

      //Своя инишка есть?
      if not FileExists(ExtractFilePath(Application.ExeName)+'IVClient.ini') then
      begin
       //нету - берем радмина
       SaveTekJrn(DateTimeToStr(now)+' : Своего ини-файла пока нету ищем от радмина.');
       ...
       begin
        ...
        SaveTekJrn(DateTimeToStr(now)+' : Нет ini файлов ВООБЩЕ!');
       end;
      end
      else
      begin
       ...
       SaveTekJrn(DateTimeToStr(now)+' : Открываем свою инишку.');
...
begin
      ...
      //Проверим - может такой узел уже есть..
      t.Filtered := False;
      if t.Locate('uzel1',name_new_usel,[]) then
      begin
       MessageBox(handle,'Такой узел уже существует!','!',0);
       goto fin;
      end;
